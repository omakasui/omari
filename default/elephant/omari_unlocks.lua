--
-- Dynamic Omari Unlocks Menu for Elephant/Walker
--
-- A "Default" entry restores the omari-shipped Plymouth via
-- omari-plymouth-reset. After that, every theme that has a preview-unlock.png
-- appears as a customised unlock; picking one runs omari-plymouth-set-by-theme
-- <theme>. Both run in a floating terminal so sudo can prompt.
--
Name = "omariunlocks"
NamePretty = "Omari Unlocks"
...
function GetEntries()
  ...
  local user_themes_dir = home .. "/.config/omari/themes"
  local omari_path = os.getenv("OMARI_PATH") or ""
  local default_themes_dir = omari_path .. "/themes"
  local default_preview = omari_path .. "/default/plymouth/preview-unlock.png"
      ...
      if theme_name and not seen_themes[theme_name] then
        ...
        if file_exists(preview_path) then
          ...
          table.insert(entries, {
            ...
            PreviewType = "file",
            Actions = {
              activate = "omari-launch-terminal-with-presentation "
                .. shell_escape("omari-plymouth-set-by-theme " .. shell_escape(theme_name)),
            },
          })
        end
      end
  ...
  local default_entry = {
    ...
    Actions = {
      activate = "omari-launch-terminal-with-presentation "
        .. shell_escape("omari-plymouth-reset"),
    },
  }
